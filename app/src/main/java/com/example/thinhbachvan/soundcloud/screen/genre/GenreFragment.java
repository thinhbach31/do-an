package com.example.thinhbachvan.soundcloud.screen.genre;

import android.content.Context;
import android.os.Bundle;
import android.support.annotation.NonNull;
import android.support.annotation.Nullable;
import android.support.v4.app.Fragment;
import android.support.v7.widget.GridLayoutManager;
import android.support.v7.widget.RecyclerView;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.example.thinhbachvan.soundcloud.R;
import com.example.thinhbachvan.soundcloud.data.model.Genre;
import com.example.thinhbachvan.soundcloud.data.repository.GenreRepository;
import com.example.thinhbachvan.soundcloud.data.source.local.GenreLocalDataSource;
import com.example.thinhbachvan.soundcloud.screen.genre.genre_detail.GenreDetailFragment;

import java.util.List;

public class GenreFragment extends Fragment implements GenreContract.View,
        GenreAdapter.onGenreSelectListener {

    private Context mContext;
    private GenreContract.Presenter mPresenter;
    private static final int AUDIO_POSITION = 0;
    private static final int LARGER = 2;
    private static final int SMALLER = 1;
    private RecyclerView mRecyclerViewGenre;
    private GenreAdapter mGenreAdapter;

    public GenreFragment(){

    }

    public GenreFragment newInstance(){
        GenreFragment genreFragment = new GenreFragment();
        return genreFragment;
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_genre, container, false);
        mContext = getContext();
        mRecyclerViewGenre = view.findViewById(R.id.recycler_genre);
        setupUI(container);
        mPresenter = new GenrePresenter(this,
                GenreRepository.getInstance(new GenreLocalDataSource()));
        mPresenter.loadGenre();
        return view;
    }

    private void setupUI(ViewGroup container) {
        GridLayoutManager gridLayoutManager = new GridLayoutManager(getContext(), 2,
                GridLayoutManager.VERTICAL, false);
        gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            @Override
            public int getSpanSize(int i) {
                return i == AUDIO_POSITION ? LARGER : SMALLER;
            }
        });
        mRecyclerViewGenre.setLayoutManager(gridLayoutManager);
        mGenreAdapter = new GenreAdapter(null, container.getContext());
        mRecyclerViewGenre.setAdapter(mGenreAdapter);
        mGenreAdapter.setOnGenreClickListener(this);
    }

    @Override
    public void showGenre(List<Genre> genres) {
        mGenreAdapter.setGenres(genres);
    }

    @Override
    public void onGenreSelected(String genre) {
        gotoDetaiFragment(genre);
    }

    private void gotoDetaiFragment(String genre) {
        getActivity().getSupportFragmentManager().beginTransaction()
                .replace(R.id.fragment_genre, GenreDetailFragment.newInstance(genre))
                .addToBackStack(null)
                .commit();
    }
}
